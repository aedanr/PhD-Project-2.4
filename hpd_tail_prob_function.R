...
hpd.pval <- function(x, m=0) {
  ...
  y2 <- numeric(9)
  for (t in 1:9) {
    y2[t] <- sign(hdi(x, credMass=1 - (iter + t * 0.01))[1] - m) == sign(hdi(x, credMass=1 - (iter + t * 0.01))[2] + m)
    if (y2[t] == 1) {break}
  }
  ...
  y3 <- numeric(9)
  for (t in 1:9) {
    y3[t] <- sign(hdi(x, credMass=1 - (iter + t * 1e-3))[1] - m) == sign(hdi(x, credMass=1 - (iter + t * 1e-3))[2] + m)
    if (y3[t] == 1) {break}
  }
  ...
  y4 <- numeric(9)
  for (t in 1:9) {
    y4[t] <- sign(hdi(x, credMass=1 - (iter + t * 1e-4))[1] - m) == sign(hdi(x, credMass=1 - (iter + t*1e-4))[2] + m)
    if (y4[t] == 1) {break}
  }
  if (length(x) <= 1e4) {
    ...
    y5 <- numeric(9)
    for (t in 1:9) {
      y5[t] <- sign(hdi(x, credMass=1 - (iter + t * 1e-5))[1] - m) == sign(hdi(x, credMass=1 - (iter + t * 1e-5))[2] + m)
      if (y5[t] == 1) {break}
    }
